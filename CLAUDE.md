...
The whole system runs as Docker Compose services (`slskd`, `workflow`, `dashboard`, `backup`) and is driven by a Radarr-style task scheduler rather than a single linear script.
...
`docs/adr/0006-complete-dashboard-cutover.md`.
...
1. Resolves `ref` to a commit and `git archive`s exactly `scripts/`, `observability/dashboard/`, `requirements.txt`, and `infra/Dockerfile.backup` into `.deploy/deployed-code/` (wiped and recreated from scratch each time), recording the commit in `.deploy/DEPLOYED_SHA`.
...
3. If the gated environment is currently hot, restarts `workflow` and `dashboard` (plain restarts, not rebuilds — their code is bind-mounted, not baked into the image) so they pick up the new snapshot.
...
Folder membership (`playlist_folder_memberships` table) is fully derived from the CSV and rebuilt from scratch on every `task_scrape_playlists` run (`folder_name = ''` means top level). `xml_exporter` reads it to emit iTunes folder `<dict>`s (`Folder`/`Parent Persistent ID`); a playlist in N folders becomes N playlist entries with deterministic persistent IDs. Folders are XML/Rekordbox-only — m3u8 files stay flat, and the dashboard's only use of folders is as a read-only scope in the Manual Import tab's dropdown (alongside `All Playlists` and individual playlists). Each folder also gets a synthesized `ALL <folder name>` master playlist (the union of its members' tracks) emitted first inside the folder — likewise XML-only, derived at export, never stored in the DB. See `CONTEXT.md` for the glossary and `docs/adr/0001-playlist-folders-in-itunes-xml.md` / `docs/adr/0007-folder-master-playlists.md` for the rationale.
