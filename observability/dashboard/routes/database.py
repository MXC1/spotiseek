...
from observability.dashboard.config import ENV, IMPORTED_DIR, track_db
...
def _track_context(track_id: str) -> dict:
    ...
    context["detail"] = {
        ...
        "file": disk_checks.file_facts(track.get("local_file_path")),
...
def run_disk_check(request: Request, check_id: str):
    ...
    def build() -> dict:
        ...
        if check_id == "completed_files_missing":
            ...
            found = disk_checks.find_missing_track_files(track_db.get_completed_track_files())
            total = len(found)
        ...
        return _disk_result(check_id, columns, found[: disk_checks.MAX_LISTED_FILES], total, note)

    return templates.TemplateResponse(request, "tabs/_database_disk_result.html", _guarded(build))


def _disk_result(check_id: str, columns: list[str], rows: list[tuple], total: int, note: str | None) -> dict:
